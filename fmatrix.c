...
double fmatrix_compute_residual_pano(double *F, v3_t l, v3_t r, double radius) 
{
    double Fl[3], Fr[3], pt1, pt2;    
	double norm;
	...
	//normalized vector
	norm = sqrt( r.p[0]*r.p[0] + r.p[1]*r.p[1] + r.p[2]*r.p[2] );		
	r.p[0] /= norm;
	r.p[1] /= norm;
	r.p[2] /= norm;

	norm = sqrt( l.p[0]*l.p[0] + l.p[1]*l.p[1] + l.p[2]*l.p[2] );
	l.p[0] /= norm;
	l.p[1] /= norm;
	l.p[2] /= norm;

	norm = sqrt( Fl[0]*Fl[0] + Fl[1]*Fl[1] + Fl[2]*Fl[2] );
	Fl[0] /= norm;
	Fl[1] /= norm;
	Fl[2] /= norm;

	norm = sqrt( Fr[0]*Fr[0] + Fr[1]*Fr[1] + Fr[2]*Fr[2] );
	Fr[0] /= norm;
	Fr[1] /= norm;
	Fr[2] /= norm;

	//dot product	
    pt1 = Vx(r) * Fl[0] + Vy(r) * Fl[1] + Vz(r) * Fl[2];
	pt2 = Vx(l) * Fr[0] + Vy(l) * Fr[1] + Vz(l) * Fr[2];
    
	//calculate the intersection angle
	interSectionAngle =  3.1415926*0.5 -  ( acos(pt1) + acos(pt2) ) * 0.5; 	
    
	//return the distance
    return fabs(radius*interSectionAngle);    
}
